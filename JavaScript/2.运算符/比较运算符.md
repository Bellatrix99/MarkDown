## 	1.概述 [#](https://wangdoc.com/javascript/operators/comparison.html#概述)
...
严格相等运算符有一个对应的“严格不相等运算符”（`!==`），它的算法就是**先求严格相等运算符的结果，然后返回相反值。**
...
比较不同类型的数据时，相等运算符会先将数据进行类型转换，然后再用严格相等运算符比较。**（1）原始类型值**
